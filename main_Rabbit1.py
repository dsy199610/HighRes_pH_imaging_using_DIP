...
from loader.dataloader import Dataset_pH_mpMRI, interpolate_MRI, Crop_ROI
...
def main_evaluate(args):
    ...
    with torch.no_grad():
        for iter, data in enumerate(loader):
            ...
            outputs_HR = F.interpolate(outputs_HR, size=(396, 396, 4), mode='trilinear', align_corners=True)
            ...
            plt.imsave(output_file, np.concatenate((outputs_SR_plot[:, :, 0], outputs_SR_plot[:, :, 1],
                                                    ...
                                                    outputs_plot, pH_clean_plot), axis=1), cmap='jet', vmin=6.5, vmax=7.5)
            output_file = '%s/%s_%s_HR.png' % (str(args.exp_dir / 'results'), rabbit[0], slice[0])
            plt.imsave(output_file, np.concatenate((outputs_HR_plot, pH_HR_plot), axis=1), cmap='jet', vmin=6.5, vmax=7.5)
...
def create_arg_parser():
    ...
    parser.add_argument('--rabbit', type=str, default=['Lumi'], help='Rabbit name')
    parser.add_argument('--slice', type=str, default=['liver'], help='slice name')
    parser.add_argument('--exp-dir', type=str, default='checkpoints/Lumi_with_HighRes', help='Path to save models and results')
